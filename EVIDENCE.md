...
Generated 2026-09-09T01:57:24 from an actual suite run: **156/159 tests passed**, **922 observations** recorded.
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13410 token window in 1.6s, cut marked
...
- `test_team.py` **[team t-test]** done -> C:\tmp\cu0i-full-20260909-a\team\teamwork\t-test\result.md
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 21705m > 1m
...
- `test_contract.py` **[goal g-accept]** turn this success into deterministic capability: python runbook.py draft C:\tmp\cu0i-full-20260909-a\contract\experts\honest-builder g-accept (then fill the TODO steps and let three verified runs promote it)
...
- `test_runbook.py` **[goal g-20260909-014532]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260909-014532]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260909-014836]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260909-014836]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
- `test_computer_session.py` **[artifact-race]** same-size/same-digest inode replacement before open and after read both refuse before reconciliation state can change
...
- `test_package.py` **[runnable]** the archive carries 127 non-test Python files (including nested support scripts), 159 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 80,000 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_invariants.py` **[sandboxes]** 195 sandbox names across 159 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 161 ms per command — 27 s before the caches — and a change to a cached path is still caught
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.8s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.1s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_live_provider.py` **[unreachable]** a refused connection failed over to the fallback in 2.06s and was logged as unreachable, instead of costing five backoffs per step forever
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '890ad21f33bc' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94631 -> 198703 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.17s, 0.18s, 0.18s, 0.17s, 0.18s, 0.23s — median 0.18s, and the last batch is not an outlier: the loop does not get slower as its own history grows
- `test_endurance.py` **[logs]** agent.log is 115 KB and rotates at 5 MB x 5 backups — a hard ceiling of 29 MB per expert, whatever happens
