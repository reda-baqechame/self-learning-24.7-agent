...
Generated 2026-09-09T16:51:05 from an actual suite run: **157/160 tests passed**, **922 observations** recorded.
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13410 token window in 2.1s, cut marked
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 22598m > 1m
...
- `test_runbook.py` **[goal g-20260909-163837]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260909-163837]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260909-164146]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260909-164146]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 80,065 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 160 tests and 215 mutations
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 46.0s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.2s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '3a3d6aff98e9' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.17s, 0.21s, 0.19s, 0.19s, 0.19s, 0.18s — median 0.19s, and the last batch is not an outlier: the loop does not get slower as its own history grows
...
- `test_endurance.py` **[soak]** 26s of continuous operation. This is minutes, not weeks: it rules out the growth that is O(total work), and it cannot rule out a leak that needs days to show.
