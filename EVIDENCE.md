...
Generated 2026-09-05T21:27:19 from an actual suite run: **154/156 tests passed**, **853 observations** recorded.
...
| 5. Improvement & governance | **proven** | 22/22 | 117 |
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13398 token window in 1.8s, cut marked
...
- `test_retention.py` **[flat]** the hot state is capped at 50 finished task(s) after 400 more were run (limit 65), so persist is bounded work forever — measured 11 ms -> 16 ms here, but the COUNT is the guarantee and the clock is only a smoke check
...
- `test_team.py` **[team t-test]** done -> [user-home]\AppData\Local\Temp\agent-suite-computer-authority-20260905\team\teamwork\t-test\result.md
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 17117m > 1m
...
- `test_contract.py` **[goal g-accept]** turn this success into deterministic capability: python runbook.py draft [user-home]\AppData\Local\Temp\agent-suite-computer-authority-20260905\contract\experts\honest-builder g-accept (then fill the TODO steps and let three verified runs promote it)
...
- `test_runbook.py` **[goal g-20260905-211751]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260905-211751]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260905-211837]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260905-211837]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
**Verdict: proven** — 22 of 22 declared tests ran and passed, producing 117 observations.

<details><summary>What the tests observed (117)</summary>
...
- `test_computeruse.py` **[browser-target]** absent, duplicate, off-origin, dialog-blocked and expired states refuse
...
- `test_package.py` **[runnable]** the archive carries 123 non-test Python files (including nested support scripts), 156 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 156 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788657205-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 80,101 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 156 tests and 60 mutations
...
- `test_invariants.py` **[execution]** 119 modules scanned; 0 raw subprocess sites outside the authority (18 declared platform-internal, each with a stated reason)
...
- `test_invariants.py` **[metering]** 119 modules scanned by AST; every function that reaches a model provider meters it (2 declared free, each with a stated reason and each still present in the source)
...
- `test_invariants.py` **[sandboxes]** 195 sandbox names across 156 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 119 modules, 156 acceptance tests
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 139 ms per command — 27 s before the caches — and a change to a cached path is still caught
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '34c1a1652cd5' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94594 -> 198694 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.15s, 0.18s, 0.18s, 0.19s, 0.18s, 0.18s — median 0.18s, and the last batch is not an outlier: the loop does not get slower as its own history grows
