...
Generated 2026-09-05T20:47:52 from an actual suite run: **154/156 tests passed**, **850 observations** recorded.
...
| 5. Improvement & governance | **proven** | 22/22 | 114 |
...
- `test_retention.py` **[flat]** the hot state is capped at 50 finished task(s) after 400 more were run (limit 65), so persist is bounded work forever — measured 12 ms -> 14 ms here, but the COUNT is the guarantee and the clock is only a smoke check
...
- `test_team.py` **[team t-test]** done -> [user-home]\AppData\Local\Temp\agent-suite-computer-clean-20260905\team\teamwork\t-test\result.md
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 17079m > 1m
...
- `test_contract.py` **[goal g-accept]** turn this success into deterministic capability: python runbook.py draft [user-home]\AppData\Local\Temp\agent-suite-computer-clean-20260905\contract\experts\honest-builder g-accept (then fill the TODO steps and let three verified runs promote it)
...
- `test_runbook.py` **[goal g-20260905-203910]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260905-203910]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260905-203949]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260905-203949]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
**Verdict: proven** — 22 of 22 declared tests ran and passed, producing 114 observations.

<details><summary>What the tests observed (114)</summary>
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788654906-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 156 tests and 56 mutations
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 158 ms per command — 27 s before the caches — and a change to a cached path is still caught
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.5s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.2s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '4b84e56e27c2' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94587 -> 198701 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.14s, 0.16s, 0.16s, 0.17s, 0.17s, 0.17s — median 0.17s, and the last batch is not an outlier: the loop does not get slower as its own history grows
