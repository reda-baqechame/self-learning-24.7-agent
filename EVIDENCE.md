...
Generated 2026-09-09T00:48:40 from an actual suite run: **156/159 tests passed**, **914 observations** recorded.
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13410 token window in 1.9s, cut marked
...
- `test_retention.py` **[flat]** the hot state is capped at 50 finished task(s) after 400 more were run (limit 65), so persist is bounded work forever — measured 12 ms -> 18 ms here, but the COUNT is the guarantee and the clock is only a smoke check
...
- `test_team.py` **[team t-test]** done -> C:\tmp\cu0r-full-20260909-a\team\teamwork\t-test\result.md
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 21635m > 1m
...
- `test_contract.py` **[goal g-accept]** turn this success into deterministic capability: python runbook.py draft C:\tmp\cu0r-full-20260909-a\contract\experts\honest-builder g-accept (then fill the TODO steps and let three verified runs promote it)
...
- `test_runbook.py` **[goal g-20260909-003538]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260909-003538]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260909-003846]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260909-003846]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 80,008 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 159 tests and 180 mutations
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 204 ms per command — 27 s before the caches — and a change to a cached path is still caught
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.6s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.2s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '01ab159aa2b0' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_docker_live.py` **[timeout]** a 60-second command under a 6-second ceiling was cut off in 8.1s, reported as a failure, and left no container behind
...
- `test_first_day.py` **[unreachable]** a base_url nothing is listening on reported FAIL in 2.4s instead of hanging on a 20-second timeout per role
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94584 -> 198704 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.18s, 0.20s, 0.20s, 0.19s, 0.21s, 0.20s — median 0.20s, and the last batch is not an outlier: the loop does not get slower as its own history grows
...
- `test_endurance.py` **[soak]** 28s of continuous operation. This is minutes, not weeks: it rules out the growth that is O(total work), and it cannot rule out a leak that needs days to show.
