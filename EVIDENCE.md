...
Generated 2026-09-08T20:16:55 from an actual suite run: **156/159 tests passed**, **912 observations** recorded.
...
| 5. Improvement & governance | **proven except skipped** | 24/25 | 154 |
| 6. Control plane & interop | **proven** | 22/22 | 150 |
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13410 token window in 1.4s, cut marked
...
- `test_retention.py` **[flat]** the hot state is capped at 50 finished task(s) after 400 more were run (limit 65), so persist is bounded work forever \x97 measured 13 ms -> 15 ms here, but the COUNT is the guarantee and the clock is only a smoke check
...
- `test_team.py` **[team t-test]** done -> [user-home]\AppData\Local\Temp\agent-suite\team\teamwork\t-test\result.md
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 21363m > 1m
...
- `test_contract.py` **[goal g-accept]** turn this success into deterministic capability: python runbook.py draft [user-home]\AppData\Local\Temp\agent-suite\contract\experts\honest-builder g-accept (then fill the TODO steps and let three verified runs promote it)
...
- `test_runbook.py` **[goal g-20260908-200348]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260908-200348]** VERIFIED by runbook weekly-artifact \x97 zero model calls
...
- `test_swarm.py` **[goal g-20260908-200655]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260908-200655]** VERIFIED by runbook left-maker, right-maker \x97 zero model calls
...
**Verdict: proven except skipped** — 24 of 25 declared tests ran and passed, producing 154 observations.
**NOT RUN HERE — test_computeruse_live.py:** set AGENT_COMPUTER_LIVE=1 for pinned network-none Chromium fixture

<details><summary>What the tests observed (154)</summary>
...
**Verdict: proven** — 22 of 22 declared tests ran and passed, producing 150 observations.

<details><summary>What the tests observed (150)</summary>
...
- `test_package.py` **[mutation-platforms]** Windows alias cleanup/8.3 spelling and POSIX directory-fd anchoring have distinct, explicit applicability
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 80,073 bytes with zero nested archives \x97 a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 159 tests and 171 mutations
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 176 ms per command \x97 27 s before the caches \x97 and a change to a cached path is still caught
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.1s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.1s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname 'b1efba8457a2' which is not this machine's, on a Windows host running python 3.14 \x97 this is not the host backend wearing a different name
...
- `test_docker_live.py` **[credentials]** three credential-shaped variables were withheld from the container by name and by value, and of the 10 variables it did receive none came from this host except the image's own \x97 both filters checked, not just the outer one
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94633 -> 198710 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.16s, 0.16s, 0.17s, 0.18s, 0.17s, 0.19s \x97 median 0.17s, and the last batch is not an outlier: the loop does not get slower as its own history grows
...
- `test_endurance.py` **[soak]** 24s of continuous operation. This is minutes, not weeks: it rules out the growth that is O(total work), and it cannot rule out a leak that needs days to show.
