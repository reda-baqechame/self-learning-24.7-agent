...
Generated 2026-09-09T03:37:00 from an actual suite run: **156/159 tests passed**, **922 observations** recorded.
...
| 5. Improvement & governance | **proven except skipped** | 24/25 | 156 |
| 6. Control plane & interop | **proven** | 22/22 | 158 |
...
- `test_harness.py` **[manifest]** 19 tools with role allowlists, 9+ gates, policies, 14 memory tiers, budgets, events, file hashes - all read from what runs
...
- `test_compaction.py` **[cliff]** five compaction rounds and the safety rule survived each one byte-identical (never entering the summarized region), and a rule appended to the constitution on disk reached the very next window verbatim \x97 typed compaction by construction: rules are files, only conversation is summarized
...
- `test_lock.py` **[hammer]** 12 threads x 40 acquisitions: every writer survived and all 480 rows landed \x97 EACCES during lockfile creation is retried as the contention it is
...
- `test_layers.py` **[L6]** finish_task refused twice with evidence, accepted only once the check passed \x97 verification is a constraint, not a suggestion
- `test_layers.py` **[L6]** a task that can never satisfy its check fails after 3 refusals, instead of looping forever
- `test_layers.py` **[L1]** per-run cost ceiling killed the task at $3.0 after 3 steps \x97 the third brake works
...
- `test_effects.py` **[fresh]** an explicit --fresh call hits the world again \x97 the ledger is a default, not a cage
...
- `test_effects.py` **[exemption]** the review exemption is per-SUBCOMMAND, not per-file: `mcp.py call` stays exempt because guarded_call gates it, while `mcp.py enable` \x97 which grants a whole toolkit and is gated by nothing \x97 now requires the owner. Checked across 7 shapes including chaining and a lookalike script.
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13410 token window in 1.3s, cut marked
...
- `test_candidates.py` **[adaptive]** one attempt until something fails, then 3, then 5 \x97 capped by the owner's setting and switchable off
- `test_candidates.py` **[explain]** the winner is reported with the reason every loser lost
- `test_candidates.py` **[wired]** the loop stashes every refused attempt (15 of them here) and promotes one only when it strictly beats the last \x97 on a task where the verifier cannot discriminate it reports a tie and changes nothing, which is the honest answer rather than a shuffle
- `test_candidates.py` **[discriminates]** a real answer scores 1.0, a one-character one 0.75, an unfinished one carrying TODO 0.75, and a .json that does not parse 0.6 \x97 the composite can now tell attempts apart on ordinary work, where every other component declines to answer and six attempts previously tied at 0.0
...
- `test_retention.py` **[lossless]** all 302 tasks accounted for \x97 260 archived, every field intact and findable by id
- `test_retention.py` **[flat]** the hot state is capped at 50 finished task(s) after 400 more were run (limit 65), so persist is bounded work forever \x97 measured 11 ms -> 13 ms here, but the COUNT is the guarantee and the clock is only a smoke check
...
- `test_use_cases.py` **[recon]** two verified weeks against a truth-recomputing gate became an unprompted candidate procedure with its parameters invented \x97 and it is NOT trusted until an owner seals fresh cases
- `test_use_cases.py` **[sentinel]** a healthy drain queued nothing; the ERROR line fired exactly one gated investigation, which passed its own mechanical gate \x97 no model watched anything
- `test_use_cases.py` **[memory]** the gate-diagnosed failure was filed by the harness and injected into the NEXT task's compiled context before it ran \x97 nobody had to remember
...
- `test_vision_preservation.py` **[task]** an ordinary gated task completed through model reasoning with zero procedures on disk \x97 a missing procedure is not a missing capability
...
- `test_vision_preservation.py` **[goal g-vision]** achieved but NOT verified: no mechanical acceptance tests exist, so nothing can be VERIFIED \x97 the honest ceiling for this contract is PARTIAL
- `test_vision_preservation.py` **[goal]** a goal was pursued to ACHIEVED under its own frozen milestone check \x97 outcomes, not steps
...
- `test_vision_preservation.py` **[frontier]** a capability this machine does not have became a structured gap with a named remediation route \x97 the tool universe stays open-ended, and 'cannot yet' never reads as 'impossible'
- `test_vision_preservation.py` **[vendor]** the provider was replaced outright; the expert's state, history and institutional memory carried over \x97 intelligence is rented, experience is owned
- `test_vision_preservation.py` **[learning]** four accepted-claimed wins with no independent receipt left the procedure at CANDIDATE \x97 trust is bought with fresh sealed evaluation, never with self-report
- `test_vision_preservation.py` **[gate]** a worker that only ever said 'done' against a failing gate produced a FAILED task on the record \x97 models never judge their own work
- `test_vision_preservation.py` **[authority]** the CONTROL-zone write was refused while ordinary work completed \x97 permissions live outside the model
- `test_vision_preservation.py` **[depth]** teams, research, mastery, memory, goals, workflows and procedural learning keep their deep tests registered in this same suite run \x97 this file pins the vision, those prove the depth
...
- `test_lanes.py` **[federation]** identity, publish, A2A card \x97 all from the panel, no secret material in any payload
- `test_lanes.py` **[shapes]** settings carry key_present; detail carries consults \x97 the panel renders what the backend actually returns
...
- `test_team.py` **[team t-test]** done -> C:\tmp\cu0i-final-suite-byte\team\teamwork\t-test\result.md
- `test_team.py` **[flow]** lead planned, both specialists delivered, lead synthesized \x97 all gated
- `test_team.py` **[handoff]** outputs flowed forward as files; no shared mutable state
- `test_team.py` **[isolation]** beta:1 task, gamma:1 task, alpha:plan+synthesis \x97 memories separate
...
- `test_goal.py` **[goal g-test]** achieved but NOT verified: no mechanical acceptance tests exist, so nothing can be VERIFIED \x97 the honest ceiling for this contract is PARTIAL
...
- `test_contract.py` **[frozen]** acceptance was frozen before any work, sealed outside the worker's root, and verify() ran the checks itself \x97 failing while the artifact was missing, passing once it existed
- `test_contract.py` **[no-vacuous]** zero acceptance tests means nothing can be VERIFIED \x97 stated in the verdict, not hidden inside an empty all()
- `test_contract.py` **[tamper]** swapping a failing grader for a passing one after the freeze produced a TAMPER verdict with nothing run \x97 and forging the snapshot's own hash lost to the seal outside the root
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 21807m > 1m
...
- `test_contract.py` **[oscillation]** the same check failing in consecutive cycles is diagnosed with the wall named; a new failure reason or a spaced repeat is not \x97 progress and looping are told apart
...
- `test_contract.py` **[goal g-accept]** turn this success into deterministic capability: python runbook.py draft C:\tmp\cu0i-final-suite-byte\contract\experts\honest-builder g-accept (then fill the TODO steps and let three verified runs promote it)
- `test_contract.py` **[outranked]** a lying judge AND a generous planner-authored check both said done while the deliverable did not exist \x97 the frozen acceptance test refused, the pursuit was overruled into cycle 2, did the work for real, and only then ended VERIFIED, with the whole story in the event ledger
...
- `test_contract.py` **[goal g-wall]** BLOCKED: milestone M1 failed the same check in cycles 1 and 2 \x97 the plan is not converging on this wall, and cycle 3 would hit it a third time. Check
- `test_contract.py` **[converge]** the same milestone failing the same check in cycles 1 and 2 ended the pursuit BLOCKED with the wall named \x97 two of the four budgeted cycles were spent, not all four
...
- `test_runbook.py` **[valid]** 6 malformed shapes refused with the defect named \x97 a step with no verify, a TODO left in, a 21-step monolith; a runbook that cannot be validated cannot be trusted to run
...
- `test_runbook.py` **[goal g-20260909-032718]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260909-032718]** VERIFIED by runbook weekly-artifact \x97 zero model calls
- `test_runbook.py` **[pennies]** goal.pursue completed a goal VERIFIED with ZERO tasks created and ZERO model calls \x97 against a provider rigged to fail any task instantly, so the model path could not have produced this outcome even by accident. The model is now reserved for goals the library has never seen.
- `test_runbook.py` **[draft]** a verified pursuit yields a skeleton carrying the proven VERIFICATIONS with the HOW left as named TODOs \x97 validation refuses to run it until they are filled, because the machine can recover what was proven but not how it was done
...
- `test_runbook.py` **[record]** a demonstrated procedure landed as a CANDIDATE with recorded provenance; a step without verify was refused; the rehearsal replayed the demo through the authority stack, was recorded as a run, and earned NO trust \x97 a procedure grading its own replay is still the procedure grading itself
- `test_repair.py` **[law1]** a blocked goal's diagnosis and every grounded repair action carry the failing check and its recorded error VERBATIM from the ledger \x97 there is no 'reflect and try again' action anywhere in the plan
- `test_repair.py` **[law3]** a budget block and a tamper block both plan exactly one action \x97 OWNER \x97 and a repair pass left the contract's budget bit-for-bit unchanged: the machine cannot lift its own ceiling or forgive edits to its own graders
- `test_repair.py` **[law4]** revising a failing runbook wrote publisher-v2 BESIDE its parent \x97 parent file and earned trust untouched, child a zero-trust candidate carrying the exact failure it must answer, refused by validation until the TODO is filled
- `test_repair.py` **[law2]** a repair whose fix was real ended VERIFIED \x97 but the ledger shows the passing verify event BEFORE the state change: the frozen graders spoke first. A repair that fixed nothing resumed and did NOT verify. Repair moves blocked->running; it never grades.
- `test_repair.py` **[bounded]** the identical repair plan was refused a second run ('not converging'), and the attempt bound turned away a repair past its limit \x97 a goal still blocked after grounded repairs gets an owner, not a fourth attempt
...
- `test_swarm.py` **[rule1]** two failing acceptance tests with NO declared groups planned NO fan-out \x97 independence is declared by the caller who wrote the graders, never guessed by the machine, because assumed separability is where the measured -39% to -70% lives
...
- `test_swarm.py` **[fanout]** two declared groups with two distinct proven procedures ran as two workers, both artifacts produced, and the state moved only after the central graders passed \x97 with the task queue untouched: multiplication of MACHINE work, zero model calls
- `test_swarm.py` **[rule4]** a group whose lease was held by another swarm was NOT run twice \x97 the worker reported the held lease, the other group proceeded; with the lease released, the single remaining group was correctly refused fan-out and finished on the sequential path instead
- `test_swarm.py` **[rule3]** both workers reported success; the central graders refused A1 and the swarm result was NOT verified, with the refusing test named \x97 a worker's opinion of its own work counts for nothing, in the contract AND in the trust ledger
...
- `test_swarm.py` **[goal g-20260909-032844]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260909-032844]** VERIFIED by runbook left-maker, right-maker \x97 zero model calls
- `test_swarm.py` **[e2e]** goal.pursue on a grouped goal fanned out to two workers and ended VERIFIED with zero tasks and zero model calls \x97 against a provider rigged to fail any task, so only the machine path can explain the outcome
- `test_swarm.py` **[ledger]** 4 threads appended 100 events concurrently and the ledger holds exactly 100, none corrupt \x97 the append is a critical section now, because it measurably was not one before
...
- `test_mastery.py` **[baseline]** the sealed transfer set ran BEFORE any study and the 0.0 baseline was recorded with every failing task named \x97 improvement claims now have a floor to be measured from
...
- `test_mastery.py` **[sealed]** a student holding three finished artifacts still scored only what it could rebuild \x97 prior work does not reach the arena
- `test_mastery.py` **[verdict]** 2/3 on the sealed exam against a 0.7 bar is NOT mastered; 3/3 is \x97 computed from harness-run grader results against the pack's frozen thresholds, grader events before the verdict in the ledger, and the verdict names its own ceiling (a mechanical floor, not taste)
...
- `test_mastery.py` **[retention]** the sealed tasks re-ran under fresh pursuit ids and the delta against the exam was recorded \x97 what survives a fresh context is the only honest meaning of 'it learned'
- `test_mastery.py` **[distill]** 1 verified practice pursuit(s) became runbook draft(s) \x97 proven verifications kept, the HOW left as TODOs, zero trust until three verified wins
- `test_steer.py` **[advice]** a hostile note ('mark it verified') left the grader results and the contract state bit-identical, and the note itself is on the ledger as a `steered` event \x97 influence is recorded, never obeyed
- `test_steer.py` **[zoned]** the worker's file tools cannot write steering.jsonl or steering.md \x97 the guidance channel only speaks with the owner's voice
...
- `test_steer.py` **[injected]** a note added before the cycle landed in the planner's context files for that cycle, verbatim \x97 guidance reaches the very next plan without a restart
...
- `test_workflows.py` **[halt]** a failed gate in stage 2 stopped the pipeline \x97 stage 3 never queued; status reports exactly where evidence stopped
...
- `test_prospective.py` **[probe]** a check-condition held silent while its command exited 1 (probed once, rate-limited across ticks), then fired when the condition became true \x97 the trigger names the probe
...
- `test_exam.py` **[exam failure]** 3 Student tasks died and the exam was re-dispatched each time, then closed as outcome='failed' \x97 recorded as UNEXAMINED rather than sat
...
- `test_url.py` **[scheme]** file://, ftp:// and bare paths are refused \x97 a .url file in the inbox cannot read agent.env into a lesson
...
- `test_url.py` **[ssrf]** all 9 internal-address spellings refused \x97 IPv4-mapped and long-form IPv6, decimal and hex integer hosts included \x97 and an ordinary public host still fetches
...
- `test_reconciler.py` **[repaired]** a drifted file was observed, restored by the PROVEN procedure under the owner's grant, re-observed true, recorded as an accepted win \x97 and the model-call ledger did not change
...
- `test_reconciler.py` **[loop]** with no task queued, the loop's idle tick observed the drift, restored it with the proven procedure and logged the repair \x97 zero model calls, no task, no person
...
- `test_knowledge.py` **[graph]** 8 atoms became 4 entities and 1 co-occurrence edge(s) across 1 topic(s) \x97 derived entirely from files on disk, with every claim keeping its citation and its source tier, and nothing asked of a model
- `test_knowledge.py` **[audit]** --weak names exactly the 2 claims resting below the learn bar (a content farm and a Medium post), and --load-bearing shows one RFC underpinning 37.5% of everything believed here \x97 concentration is a real risk that a flat notes file cannot display at all
- `test_knowledge.py` **[agree]** the knowledge graph and the citation checker see the identical 4 atoms across 4 course layouts up to 4 levels deep, because they call one walker rather than two \x97 the flat path this used to join matched nothing the platform writes, and both this test and the code were wrong the same way
...
- `test_memory.py` **[classify]** 8 harness errors mapped to fixed categories deterministically \x97 no model asked to guess why it failed
...
- `test_memory.py` **[retired]** retirement moves a whole world aside intact \x97 queryable, listed, and restorable years later
- `test_memory.py` **[preserve]** delete retires by default; only an explicit purge destroys
- `test_memory.py` **[automatic]** finishing a task files its own competence outcome and, on failure, a categorized failure record \x97 retries count as occurrences but never as extra competence attempts
...
- `test_skillgraph.py` **[banner]** injected skills carry their earned status \x97 the model knows hypothesis from proven procedure
...
- `test_recall.py` **[archive]** compaction removed 50 turns from the window; all 50 archived verbatim \x97 context is never lost
...
- `test_associative.py` **[empty]** no anchors, no expansion \x97 recall stays quiet
...
- `test_awareness.py` **[evidence]** one lucky success is reported as insufficient evidence, and a playbook whose ablation showed measured harm is named do-not-use \x97 losses alone no longer convict
...
- `test_awareness.py` **[institutions]** 22 real public bodies \x97 the EU, CERN, Max Planck, INRIA, RIKEN, CSIRO, four national governments, the UN, OECD, IEEE, ISO, MIT OpenCourseWare \x97 all clear the learn bar, while 5 content-farm and lookalike URLs do not; before this, the European Commission rated exactly the same as an SEO blog
...
- `test_cases.py` **[fixed]** a later task that passed its gate closed the case, and what it did is recorded as the fix \x97 verified by the gate, not by an opinion
- `test_cases.py` **[recurred]** the same failure after a fix was recorded as RECURRED \x97 the ledger now says the obvious fix already failed once
...
- `test_cases.py` **[ledger]** 1 case(s), 1 that came back after a 'fix' \x97 the number a team actually needs to see
...
- `test_gotcha_retire.py` **[specific]** a different command proved nothing, and pandoc failing again proved nothing \x97 the gotcha still binds
- `test_gotcha_retire.py` **[retired]** a later step ran pandoc successfully and the gotcha was withdrawn \x97 MARKED, not deleted: the line still carries its cause, the date, and the task that disproved it
...
- `test_gotcha_retire.py` **[resurrection]** the same failure came back after being withdrawn \x97 the gotcha binds again, and the line permanently records that it was disproved once and returned, which is the signature of a FLAPPING environment rather than a fixed one
- `test_gotcha_retire.py` **[conservative]** a failure with no runnable subject gets no probe and never auto-retires, and gotcha files written before probes existed still parse and still bind \x97 under-retiring costs a context slot, over-retiring deletes a warning that was still true
...
- `test_discover.py` **[degrade]** two dead rails and one that raised KeyError left the live rail's result intact, each failure named with its reason \x97 a catalogue outage is a partial answer, not a total one
- `test_discover.py` **[no-trash]** all 4 search-engine result pages are tier 4 by host and cannot clear the learn bar at any setting \x97 'only reputable sources' is a property of the catalogue, not an instruction a model may ignore
...
- `test_discover.py` **[relevance]** the goal was reduced to its subject before being sent ('b-tree index concurrency control'), and 2 confidently-irrelevant tier-1 result(s) were dropped and counted \x97 an off-topic source reached by a trusted route becomes a cited atom, which is a wrong belief carrying a real citation
...
- `test_sources.py` **[provenance]** DOI resolvers, Crossref, DataCite and arXiv rate tier 2 \x97 real scholarly provenance, still learnable, and the reason states outright that an identifier is not a review mark
- `test_sources.py` **[honest]** none of the 12 unreviewed scholarly hosts carries a why-text claiming review \x97 the words a human reads now match what the host actually promises
- `test_sources.py` **[fed]** all 8 scholarly discovery rails still produce results at or under the learn bar \x97 the halo is gone, the pipeline is not
...
- `test_sources.py` **[derived]** a tier and an owner override forged in the workspace ledger are both ignored \x97 the tier is recomputed from the URL \x97 while a genuine set_tier still overrules, recorded in a control-zoned file the agent cannot write and the workspace ledger stays writable so ingestion keeps working
...
**Verdict: proven except skipped** — 24 of 25 declared tests ran and passed, producing 156 observations.
**NOT RUN HERE — test_computeruse_live.py:** set AGENT_COMPUTER_LIVE=1 for pinned network-none Chromium fixture

<details><summary>What the tests observed (156)</summary>
...
- `test_variants.py` **[tie]** equal performance refused \x97 churn without evidence is rot
...
- `test_replay.py` **[swap]** a different model measured at 33% agreement with 1 drift and 1 refusal \x97 decisions read, never executed, state untouched
- `test_benchmark.py` **[bare]** the model's first answer failed all 3 mechanical checks \x97 and reported success on every one of them
...
- `test_modelrouter.py` **[attribution]** a failover failure was split 0.9/0.1 by served share: the cheap model that did nine steps carries nine tenths of the failure, the fallback that finished it carries one tenth \x97 the router's economics are no longer polluted by whoever happened to serve last
...
- `test_tabular.py` **[unit]** ambiguity refuses: ragged, unknown, colliding, oversized, unused sources \x97 none of them guess
...
- `test_operator_runtime.py` **[rollback]** a mutation whose declared effect did not hold was rolled back whole \x97 the database is asserted or untouched, never in between
- `test_operator_runtime.py` **[authority]** a db step demands the owner's token for exactly its file \x97 derived from what the bound step touches, never declared away; granted, the same step runs
...
- `test_operator_runtime.py` **[3 migration]** a SQL migration whose commit is gated on key preservation went proven on sealed fresh databases (materialized from owner-sealed scripts) and replayed week three with zero model calls \x97 the gate re-diffed staging against archive itself
...
- `test_operator_runtime.py` **[5 csv->sql]** a two-step mixed pipeline \x97 typed normalize, then gated load whose sum is conserved \x97 went proven and replayed with zero model calls across BOTH state domains in one procedure
- `test_verifier_factory.py` **[factory]** a worker manufactured a gate proposal \x97 provenance stamped, status CANDIDATE \x97 and the candidate gating a task FAILED it, fail-closed: nothing the model makes can grade the model
- `test_verifier_factory.py` **[falsifiable]** a calibration set with nothing to REJECT was refused \x97 a verifier that cannot fail anything is not a verifier
- `test_verifier_factory.py` **[discrimination]** a verifier that accepted a case it was required to reject earned nothing \x97 promotion refused, still candidate
- `test_verifier_factory.py` **[gates]** the trusted verifier passed a correct report and FAILED an off-by-one-cent one, live, with verification recorded and the verdict path free of shell and model \x97 and the gated task opened a learning trajectory
- `test_verifier_factory.py` **[hash-bound]** editing the trusted spec demoted it to candidate and gating refused again \x97 trust binds to exact bytes
- `test_verifier_factory.py` **[names]** a worker proposing over an existing name was refused \x97 no proposal can shadow another's gate
- `test_procedure_v2.py` **[validate]** unknown kinds, unbounded loops, over-cap retries, self-calls, model steps and over-deep nests all refuse \x97 the IR is closed and total
- `test_procedure_v2.py` **[if]** the predicate was OBSERVED at run time and both arms were exercised \x97 branching is a fact about the world, not a guess
...
- `test_procedure_v2.py` **[compensate]** the cleanup ran, its own effects verified \x97 and the procedure STILL failed: compensation is never success
...
- `test_capability_signatures.py` **[identity]** the signature is computed from schema, operators, effects and control \x97 rewording changes nothing, restructuring changes everything
- `test_capability_signatures.py` **[paraphrase]** a task sharing no words with the proven procedure was found by STRUCTURE and logged structural_only \x97 while the task still ran through the model, because shadow has no authority
...
- `test_capability_signatures.py` **[strict]** wrong typed inputs proposed nothing \x97 the structural lens does not guess either
- `test_capability_signatures.py` **[ledger]** the report counted one structural-only find, one agreement and one strict refusal \x97 and says in as many words that authority stayed lexical
- `test_capability_signatures.py` **[collision]** two proven procedures with one typed schema and one operator/effect shape \x97 write a digest, write a purge \x97 share a signature and were BOTH proposed for a paraphrase: schema compatibility is not semantic identity, which is why the shadow holds no authority
- `test_capability_signatures.py` **[shadow-failure]** a structural check the guard had to drop was logged as signature_shadow_failure with its error class \x97 the miss is data, not silence \x97 while the live route completed the task untouched
...
- `test_git_operators.py` **[restore]** a commit whose declared effect did not hold left HEAD, refs, index and worktree exactly as before \x97 asserted or untouched, never between
...
- `test_git_operators.py` **[tamper]** a planted pre-commit hook and an edited .git/config each made every operation refuse \x97 fail closed \x97 and the hook never ran; the canonical control files restored, work resumed
- `test_git_operators.py` **[clean-index]** a commit from a dirty index refused with the staged change untouched; from a clean index the commit held exactly the declared path \x97 read by the host's git, not the adapter \x97 and the ref digest ignores the worktree, as its name says
- `test_git_operators.py` **[authority]** a git step demands the owner's token for exactly its repository \x97 per leaf (v2) and per static walk (v1) \x97 never declared away; the worker tool refuses outside git_write
...
- `test_xlsx_operators.py` **[byte-evidence]** two workbooks that decode to the same text but differ in bytes carry different trajectory evidence \x97 hashed as the bytes they are
...
- `test_correctness_patch.py` **[hash]** a 3 MiB file hashes identically to hashlib over its bytes, and still does when the whole-file reader is replaced with one that raises \x97 evidence is streamed, never loaded
...
- `test_http_operators.py` **[idempotency]** the same effect in the same lineage was replayed from the effects ledger \x97 the fixture saw one PUT
- `test_http_operators.py` **[credentials]** the bearer reached the fixture and nothing else \x97 not the tool output, not the effects ledger; a redirect was refused, not followed
...
- `test_http_operators.py` **[end-to-end]** record syncs compiled a candidate, went PROVEN on a sealed fresh suite (edge: empty name, unicode tags), and replayed record nine with zero model calls under an independent gate that read the fixture directly \x97 one PUT, with an idempotency key
- `test_computeruse.py` **[browser-selector]** a click selector reaches the server; file and private-network URL targets still refuse
- `test_computeruse.py` **[content]** same-length corruption, truncation and excess bytes refuse
- `test_computeruse.py` **[exact-artifacts]** three exact synthetic invoices verify without certifying browser authority or release
- `test_computeruse.py` **[hardlink]** an artifact aliased outside its output directory refuses
- `test_computeruse.py` **[manifest-binding]** changed expectations cannot reuse the trusted manifest pin
- `test_computeruse.py` **[exact-set]** missing, extra, partial and duplicate artifacts refuse
- `test_computeruse.py` **[containment]** absolute, parent, empty-component and drive-relative output paths refuse
- `test_computeruse.py` **[manifest-schema]** duplicate IDs, traversal, boolean sizes, oversize, invalid month and digest refuse
- `test_computeruse.py` **[invoice-identity]** matching bytes alone cannot substitute another invoice identity or month
- `test_computeruse.py` **[blocked-observation]** blocked and empty states can be sealed for recovery but refuse click before consumption or adapter entry
- `test_computeruse.py` **[post-observation]** a dispatched click without fresh post-observation remains unknown
- `test_computeruse.py` **[blocked-post-state]** acknowledged dialog/auth transitions preserve ACTION_DISPATCHED and observation; malformed post-state remains unknown
- `test_computeruse.py` **[browser-action]** exact preconditions execute once; uncertain adapter failure cannot retry
- `test_computeruse.py` **[browser-receipt]** forged, superseded, restarted-session and expired observations refuse
- `test_computeruse.py` **[playwright-adapter]** host locator executor is identity-bound, denies generic raw code, and refuses precondition or malformed readback
- `test_computeruse.py` **[browser-target]** duplicate, off-origin and invalid-geometry observations refuse
- `test_computer_session.py` **[pending-effects]** acknowledgment permits another intent, never verifies or repeats the same unresolved business action
- `test_computer_session.py` **[cleanup-all]** first failure retains its tainted lease while every other owned environment is still closed
- `test_computer_session.py` **[agent-finally]** external cancellation/failure refuses next action and closes; interrupted runtime finally closes child
- `test_computer_session.py` **[agent-tools]** direct role denial, typed observations, one process across turns, blocked lease retained, completion closes
- `test_computer_session.py` **[artifact-race]** an open identity anchor blocks or detects same-bytes replacement before read; post-read replacement also refuses before reconciliation state can change
- `test_computer_session.py` **[daemon-binding-contract]** simulated daemon switch cannot redirect cleanup; changed daemon ID refuses before removal, not live daemon-failover proof
- `test_computer_session.py` **[docker-endpoint-policy]** forwarded endpoint/context/config/TLS selectors refuse before any subprocess launch
- `test_computer_session.py` **[empty-role-allowlist]** explicit empty tool allowlist denies direct computer tool entry
- `test_computer_session.py` **[server-quarantine]** failed construction persists server ownership across lineage changes until independent cleanup succeeds
- `test_computer_session.py` **[path-identity]** actual platform alias, typed zones, ambiguous spellings, cross-zone/outside links, secret hardlink and ordinary writes follow one authority contract
- `test_computer_session.py` **[configuration-source]** deleting pinned local config refuses identical-content fleet fallback
- `test_computer_session.py` **[navigation-scope]** off-origin post-navigation observation cannot be labeled known-no-effect refusal
- `test_computer_session.py` **[owned-process-tree]** real grandchild loses execution on close and abrupt owner death, including a child that stopped reading stdin
- `test_computer_session.py` **[owner-intervention]** worker entry and worker-only evidence denied; physical digest checked; UNKNOWN history retained with owner attestation
- `test_computer_session.py` **[session-package]** root runtime ledger and debug evidence excluded from real archive; module ships
- `test_computer_session.py` **[persistent-lock-health]** old advisory locks never called safe-to-delete; legacy stale diagnosis and live exclusion remain
- `test_computer_session.py` **[owned-chromium]** NOT_RUN: set AGENT_COMPUTER_LIVE=1 for network-none pinned Docker fixture
- `test_computer_session.py` **[posix-contract-only]** simulated syscall boundary preserves leader until kill and refuses lingering group; not native POSIX proof
- `test_computer_session.py` **[posix-wait-contract-only]** exit observation requests WNOWAIT and never invokes reaping poll
- `test_computer_session.py` **[artifact-receipts]** initial and post-click receipts bind image metadata; altered physical bytes refuse before next input
- `test_computer_session.py` **[durable-boundary]** actual fsync completes for PREPARED and DISPATCHED before adapter may send input
- `test_computer_session.py` **[crash-boundaries]** killed real owners recover PREPARED as no-effect and DISPATCHED as UNKNOWN; owned environment closure confirmed
- `test_computer_session.py` **[fresh-role-policy]** changing the owner settings file revokes the next direct public call and closes its session
- `test_computer_session.py` **[interrupt-primary]** failure to refresh quarantine cannot mask system interruption; prior durable environment still excludes new claimants
- `test_computer_session.py` **[current-task-identity]** replacement task object cannot silently inherit another lineage session
- `test_computer_session.py` **[session-lifecycle]** real process and tab reused through pause and model failover; exclusive lease, new epoch and child cleanup
- `test_computer_session.py` **[session-policy]** role and origin refuse; live owner revocation closes; effects use explicit task identity
- `test_computer_session.py` **[run-primary-exception]** public run preserves cancellation, exit code and ordinary failure while every session cleanup is attempted and diagnosed
- `test_computer_session.py` **[run-success-cleanup]** successful main cannot suppress cleanup failure; verified clean finalization preserves its return value
- `test_computer_session.py` **[system-interruption-cleanup]** KeyboardInterrupt is preserved only after every later owned session is attempted
- `test_computer_session.py` **[unavailable-view-context]** missing measurements remain null, never fabricated screenshot-coordinate authority
- `test_computer_session.py` **[unknown-lineage]** lost response taints and survives retry task identity and model failover
- `test_computer_session.py` **[cleanup-failclosed]** unproven environment cleanup retains taint and exclusive lease
- `test_computer_session.py` **[view-context]** sealed provenance cannot be altered; changed host viewport refuses before CSS-locator input
- `test_computerbench.py` **[computerbench-contract]** sealed external owner trust, frozen run settings, all five variant dimensions and separate browser/desktop/API tracks are required; synthetic tests do not set acceptance or release true

</details>

**Blind spot.** promotion and routing decisions are proven against seeded outcome ledgers, not against months of real measured performance. The design gate checks mechanics and the known fingerprints of generated filler; it cannot judge beauty. ComputerBench repository tests validate only an external-pack contract and known development fixtures: without a separately supplied owner-authenticated pack/results, independent acceptance and release remain false.

## 6. Control plane & interop

*panel, live events, cards, chief, doctor, preflight, backup, providers, MCP, A2A federation, traces*

**Verdict: proven** — 22 of 22 declared tests ran and passed, producing 158 observations.

<details><summary>What the tests observed (158)</summary>
...
- `test_ui.py` **[learner-input]** invalid cycles refuse before expert or goal state is created
...
- `test_ui.py` **[mission-work]** saving defined the contract; a separate checked action bound C1, queued its task and started the agent
...
- `test_ui.py` **[interrupt]** the owner's stop button moved a running pursuit to BLOCKED with the reason named on the ledger \x97 resumable, never silent
...
- `test_csrf.py` **[rce]** a free-form shell done_check over HTTP is refused \x97 defence in depth, even from a same-origin caller
- `test_csrf.py` **[gates]** a named gate becomes the command; a traversing parameter inside one is still refused
- `test_csrf.py` **[goal-rce]** goal acceptance over HTTP also requires named gates; a raw command is refused before any goal state exists
...
- `test_frontend.py` **[serve]** page served from ui.html (346386 bytes)
...
- `test_package.py` **[secrets]** 423 archive members checked four ways \x97 by basename, by containing directory, by extension, and by READING every text member for assigned credential values. The content scan was calling a path-taking function on a line of text, so it had never evaluated true; now live, it finds exactly the 9 synthetic fixtures the tests are built from and nothing else, and an unlisted hit or a stale exemption both fail
- `test_package.py` **[private]** none of 5 private-data shapes carries CONTENT in the archive \x97 no expert memory, task state, logs, context windows or organization roster \x97 while 8 empty placeholder(s) keep the working directories so a fresh unzip runs with no setup. Proof observations DO ship, deliberately: every one is bound to a code hash and none names this machine, so the recipient inherits evidence that falls the moment they change the code
- `test_package.py` **[runnable]** the archive carries 127 non-test Python files (including nested support scripts), 159 tests, the prompts and settings.toml \x97 and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[clone-dirs]** all 10 working directories survive a git clone, so the clone route and the zip route land the same tree \x97 a real clone of the published repo had 8 of them missing, including the inbox the installer tells the owner to use
- `test_package.py` **[harness-safety]** the mutation harness plants a decoy only where no real credential file exists, and reports None otherwise \x97 so the cleanup that follows can never remove an owner's keys, which it did while announcing that it was skipping them
- `test_package.py` **[mutation-platforms]** Windows alias cleanup/8.3 spelling and POSIX directory-fd/path-open anchoring have distinct, explicit applicability
- `test_package.py` **[runner-terminal]** exactly one last machine record controls verdicts; child footer/failure spoofing, duplicates, truncation and order drift refuse
- `test_package.py` **[runner-forgery]** child headers/records and control/newline variants are escaped before relay; truncation before the real parent record refuses, and invalid bytes remain visible without U+FFFD loss
- `test_package.py` **[planted]** 3 decoy credential file(s) were created in the source tree and the archive excluded every one, by file and by value \x97 an exclusion rule is only worth what it catches
- `test_package.py` **[evidence]** all 159 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_package.py` **[interleave]** a green test whose OK drifted under the next header stays green (verdict from exit codes, observations from what could be attributed) \x97 every comma-separated tail failure stays red and derived totals must equal the authoritative footer \x97 and spaced observation labels are counted while colon notes are not
...
- `test_chief.py` **[quiet]** an untroubled fleet gets one calm ADVANCE \x97 no invented urgency
...
- `test_mcp.py` **[fence]** tool output \x97 including a live injection attempt \x97 arrives fenced as DATA under the grounding contract
...
- `test_mcp.py` **[url-args]** 6 tool arguments pointing at file://, loopback, private and link-local addresses are refused BEFORE the server is called \x97 including nested ones, which is how a browser server passes its options \x97 and 4 ordinary argument shapes still pass
- `test_mcp.py` **[mcp-image]** different same-index images retain literal bytes at distinct SHA-256 paths; duplicates do not rewrite; PNG/JPEG/WebP dimensions are parsed, GIF stays null, and malformed/oversized payloads are refused
- `test_mcp.py` **[mcp-containment]** redirected tmp/artifact directories refused through real junction/junction filesystem entries
- `test_mcp.py` **[mcp-containment]** existing digest hard-link/junction refused without changing its outside target
- `test_mcp.py` **[mcp-race]** junction swap before unique-temp creation fails closed and cleans the outside temporary file
- `test_mcp.py` **[mcp-race]** directory replacement at atomic publication is blocked or fails closed without an outside artifact
- `test_mcp.py` **[mcp-posix-anchor]** NOT-APPLICABLE on Windows: publication uses identity revalidation and alias cleanup instead
- `test_mcp.py` **[mcp-race]** normal-path existing regular target swapped to an outside hard link during read fails closed
- `test_mcp.py` **[mcp-race]** actual-8.3-path existing regular target swapped to an outside hard link during read fails closed
- `test_mcp.py` **[mcp-return-gate]** parent move/junction during existing target open is refused after byte verification
- `test_mcp.py` **[mcp-binary]** literal WAV retains canonical .wav/audio metadata
- `test_mcp.py` **[mcp-binary]** generic bytes use safe .bin/octet-stream with null dimensions
- `test_mcp.py` **[mcp-structured]** long display text truncates, but exact structured path/hash/bytes/mime/dimensions remain available
- `test_mcp.py` **[mcp-signature]** unlabelled PNG derives .png/type/dimensions from bytes
- `test_mcp.py` **[mcp-signature]** PNG bytes declared as JPEG are refused before storage
- `test_mcp.py` **[mcp-size]** encoded-length precheck refuses a >25 MB decoded payload before base64 allocation
...
- `test_federation.py` **[evidence]** a peer's answer is filed as fenced, attributed, untrusted evidence \x97 never as our own knowledge
...
- `test_federation.py` **[digest]** hard constraints extracted, hashed, and echoed \x97 a handoff that drops them changes the hash and is detectable
- `test_providers.py` **[transaction]** concurrent public add + set_role preserve both edits and all original settings
- `test_providers.py` **[interruption]** concurrent failed replacements preserve original and foreign temp; unique temps fsynced and cleaned
- `test_providers.py` **[live-owner]** old mtime cannot steal a paused provider transaction; resumed edits both survive
- `test_providers.py` **[dead-owner]** terminating a provider process releases exclusion without stale-time takeover
...
- `test_backup.py` **[portable]** the RESTORED expert was driven through a gated task in its new location and passed \x97 deployment is a location choice, not a different expert format (manual \xa720)
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 80,015 bytes with zero nested archives \x97 a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
- `test_backup.py` **[pull]** a good archive downloads and verifies; one flipped byte deep inside is caught and REFUSED with the reason \x97 the check the feature advertised now actually runs, having previously crashed on every archive and, once unpacked, trusted a damaged one
- `test_backup.py` **[never-raises]** verify() reports zlib.error, BadZipFile, OSError and MemoryError as a NAMED corrupt member instead of raising \x97 the layer that notices damage differs by platform, and the narrow except list was green on 4 of 6 runners
...
- `test_preflight.py` **[one-truth]** a keyless fleet is NOT READY on BOTH surfaces, with preflight quoting doctor's own blocking item verbatim; supplying the key cleared both at once \x97 one computation, two renderers, no second opinion
...
- `test_ecosystem.py` **[race]** two processes evaluated the same due intention \x97 it fired exactly once (the measured double-fire stays dead)
- `test_ecosystem.py` **[writers]** 2 processes x 20 outcomes: all 40 recorded \x97 the graph lock loses nothing
- `test_ecosystem.py` **[recall]** the decision pulled its cited atom's definition across files \x97 chain, not fragment
- `test_ecosystem.py` **[variants]** trial refused while a loop pulses \x97 arms cannot be contaminated by a foreign claimer
- `test_ecosystem.py` **[chief]** the briefing surfaced the blocked question with its actual text, ranked first
- `test_ecosystem.py` **[retire]** the whole organism \x97 graph, intentions, notes \x97 survived retirement and came back byte-true
- `test_ecosystem.py` **[doctor]** full inspection: ledgers parse, no stale locks, the briefing compiles \x97 nothing wrong but the missing key
...
- `test_ledger_defects.py` **[panel]** the task dialog names a gate from the catalogue (exists, designcheck, citecheck, verify, memcheck) with one parameter; goal graders use the same catalogue, preserve owner-facing meaning, and raw, malformed, or excess graders are refused
- `test_ledger_defects.py` **[goal-input]** goal spend, time and cycle limits are finite and in range before any state is written; zero retains its documented no-extra-cap meaning
- `test_ledger_defects.py` **[goal-contract]** direct callers must provide unique string ids, stated criteria, command strings and valid optional groups before any goal state is written
- `test_ledger_defects.py` **[goal-identity]** contract ids, expert slugs and objectives are path-safe and non-empty; direct pursuits validate identity and limits before their first artifact
- `test_ledger_defects.py` **[mission-work]** queued mission work names its open criterion, expected evidence and catalogue gate; missing gates and unrelated criteria are refused before a task is queued
- `test_ledger_defects.py` **[panel-truth]** quality copy names evidence instead of invented rates; mission save and execution are separate; goal and mission context have shareable Back/Forward routes
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 159 tests and 193 mutations
...
- `test_invariants.py` **[execution]** 123 modules scanned; 0 raw subprocess sites outside the authority (19 declared platform-internal, each with a stated reason)
...
- `test_invariants.py` **[ledgers]** all 10 ledgers harness treats as integrity invariants are CONTROL and refused to the agent \x97 including skills/graph.json, the skill trust graph, which sat in the workspace because `skills/` is legitimately the agent's own
- `test_invariants.py` **[traversal]** 12 escape spellings (posix, windows, UNC, mixed, nested) all refused or contained; 4 typed dot spellings fail closed
...
- `test_invariants.py` **[metering]** 123 modules scanned by AST; every function that reaches a model provider meters it (2 declared free, each with a stated reason and each still present in the source)
...
- `test_invariants.py` **[sandboxes]** 195 sandbox names across 159 test files, every one claimed by exactly one file \x97 a shared temp directory is the failure that only shows up under load
- `test_invariants.py` **[settings]** 70 [agent] key(s) read across the modules, every one of them declared in settings.toml \x97 the file an operator reads is the file the code obeys
- `test_invariants.py` **[cli]** 107 documented subcommands across 54 modules all parse, and every module prints its own --help on a non-UTF-8 console
- `test_invariants.py` **[clocks]** every .py in the platform parsed: no file timestamp is compared against another file's, which is the comparison a coarse filesystem tick corrupts (U19, U20). The 5 remaining getmtime sites sort, or measure age against the wall clock \x97 sound, but not unconditionally: an age can come back NEGATIVE when the two clocks disagree, which is what U22 was, so this check bans the pattern it can prove and the docstring records the edge it cannot
...
- `test_invariants.py` **[org-policy]** all 3 organization policy flags are declared with an enforcer, the named module really reads each one, flipping agents_may_install actually refuses an install, and only the owner can change it \x97 all three were inert, unreachable and shown in the panel
- `test_invariants.py` **[arch-table]** all 5 rows of ARCHITECTURE.md's control table match execution.describe() flag for flag \x97 the doc gave capability_probe an approval the code never implemented
...
- `test_invariants.py` **[grants]** 10 authority classes grantable, 1 deliberately ask-every-time, 0 undecided \x97 the two vocabularies cannot drift apart silently
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 123 modules, 159 acceptance tests
- `test_promotion_leakage.py` **[constants]** the 4 module path constants and every harness ledger are pinned to the enumeration \x97 a renamed ledger cannot slip out of it
- `test_promotion_leakage.py` **[static]** all 41 trust-defining paths classify CONTROL or RUNTIME \x97 none is workspace
...
- `test_promotion_leakage.py` **[seals]** procedure authority, the verifier registry and the goal contract seal resolve under org/ \x97 CONTROL, refused to the agent actor \x97 where the worker cannot reach them
- `test_controlplane.py` **[matrix]** 131 shell commands from a role holding run_command \x97 truncate, append, delete, create and a redirect \x97 against all 40 control paths fileauth declares: not one durable change, every attempt reported exit=3, 131 tamper events on the record
...
- `test_controlplane.py` **[premise]** in the SHIPPED settings.toml, 4 role(s) hold run_command (default, examiner, practitioner, ripper) and 5 do not (consultant, librarian, reflector, student, watcher) \x97 so the matrix above attacks the real configuration; and the seal brackets all 3 model-authored operations, because a done_check is written by the model as surely as a command is
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 136 ms per command \x97 27 s before the caches \x97 and a change to a cached path is still caught
...
- `test_proof.py` **[derived]** the ledger stores observations only \x97 there is no level field for anyone to set by hand
...
- `test_proof.py` **[regression]** editing the code dropped OFFLINE VERIFIED -> IMPLEMENTED automatically, and restoring it brought the level back \x97 nobody touched a status
- `test_proof.py` **[failure]** a failing run is recorded as failing \x97 the ledger keeps both, so a regression is visible rather than overwritten
- `test_proof.py` **[expiry]** live and stress evidence older than its window expired automatically and the badge fell back to OFFLINE VERIFIED \x97 a green light cannot rot into a lie by sitting still
...
- `test_mission.py` **[amendment]** the objective cannot be edited in place \x97 the change carries a reason, an author, and both fingerprints, so drift is visible instead of silent
- `test_mission.py` **[gaps]** 4 blocker dimensions classified and routed; only the authority gap escalated to the owner
- `test_mission.py` **[every-role]** practitioner, student and consultant all receive the objective, the binding constraints and their criterion \x97 the memory router cannot route the assignment away
...
- `test_mission.py` **[unblock]** a raised blocker can be resolved through the CLI with the reason recorded, and a bad index fails loudly \x97 resolve_blocker was written but unreachable from every surface
- `test_metrics.py` **[sources]** 12 metrics read from 12 distinct ledgers, each naming its own \x97 no metric keeps a second count of something another subsystem already knows
...
- `test_metrics.py` **[honesty]** 4 metric(s) this platform cannot compute are named with the reason, rather than dropped or approximated \x97 including one that would have been flattering to invent
- `test_metrics.py` **[reliability]** 2/5 gated tasks passed and 15/17 finish-claims were refused \x97 both derived in one pass over one ledger, so neither can exceed 100% or contradict the other
- `test_metrics.py` **[autonomy]** the figure names itself an upper bound, and it MOVES: appending one approval_required event took it from 5/5 to 4/5 \x97 it reads the log, where a human being needed is actually recorded
- `test_metrics.py` **[fidelity]** 1/1 recorded actions name the criterion they serve \x97 the platform refuses to record one that does not, so this metric can only ever be 100% or reveal a bug
...
- `test_evalsuite.py` **[graders]** all 24 checks pass a hand-written correct answer and all 24 reject a deliberately wrong one \x97 both directions, because a check that only ever passes is the dead check this project already found once in its own packaging test
- `test_evalsuite.py` **[honest-stats]** 9/12 reports 47%-91%, and even a perfect 12/12 reports a lower bound of 76% \x97 a small suite is never allowed to claim certainty
- `test_evalsuite.py` **[sealed]** every holdout run is recorded against the code hash that produced it \x97 2 looks across 2 versions here \x97 because a held-out set spent without anyone counting is just training data nobody labelled
...
- `test_workers.py` **[isolation]** free work went to the disposable container, not to the equally-free, faster-starting organization machine \x97 blast radius outranks speed
...
- `test_org.py` **[solo]** with no organization created, every capability is available \x97 adding RBAC must not make a person ask themselves for permission
...
- `test_org.py` **[refusals]** a denial names the actor's role, the role required, and what to do next \x97 a refusal nobody understands is one they route around
...
- `test_org.py` **[audit]** 7 mutations recorded, each naming the actor, the action, the object and the before/after \x97 'every mutation attributable' is a query, not an aspiration
- `test_org.py` **[escalation]** an operator could not promote itself and a builder could not invite an admin \x97 the permission needed to change permissions is itself gated
- `test_rbac.py` **[solo]** with no organization, creating an agent and driving it still works with no token and no role \x97 adding RBAC must not make the person who owns the machine ask themselves for permission
...
- `test_rbac.py` **[coverage]** 23 POST routes; 20 named in the table and 3 falling through to 'create_agent', which needs builder or above \x97 a route added tomorrow is refused for a viewer, not waved through
- `test_rbac.py` **[audit]** a request that claimed a different author was recorded against the token's real owner (owner@example.com) \x97 the trail is attributable because the identity comes from the credential
...
- `test_frontier.py` **[tamper]** an edited spec AND an appended conflicting seal both return 2 with the execution ledger unchanged \x97 first-seal-wins, so appending a seal is not a way to re-seal a probe
- `test_frontier.py` **[zone]** frontier/ is a CONTROL directory and its ledger is in harness.LEDGERS \x97 the scope proved here is the write_file tool's classification, which is what an agent's file writes go through
...
- `test_frontier.py` **[anchor]** four unanchored quotes refused \x97 a capability must quote a real span of what was actually asked, or it is a capability somebody imagined
...
- `test_frontier.py` **[contained]** a RED observed on the host is recorded honestly as uncontained and is refused as the basis for installing anything \x97 the same refusal acquire.install already makes
...
- `test_frontier.py` **[no-false-ready]** a goal the hint table never knew now names 4 capability(ies) on its SECOND encounter, each reported not-ready with the recorded reason \x97 the first shape of the bug was a silent drop, the second a silent READY
...
- `test_frontier_live.py` **[target]** the sealed probe, acquire's naming and the directory on disk all name 'capabilities/ulid-py' \x97 a probe that tests a different directory from the one the install writes proves nothing
- `test_frontier_live.py` **[adopt]** a PROVEN capability is not yet READY to any agent, and adoption refused without a granted approval \x97 leaving a pending one for a human, which is the only thing that can publish it
...
- `test_training.py` **[verifier]** a candidate evaluated with a different verifier was refused \x97 comparing those numbers would measure the verifier, not the model
- `test_training.py` **[gate]** a change below its declared threshold, a single-seed result and a self-declared score were all refused; promotion required a policy sealed before the candidate existed, a paired three-seed run of the owner's frozen evaluator over 20 sealed held-out tasks, and an all-must-pass canary on 20 fresh ones \x97 and the record carries checkpoint, verifier hash, holdout hash and evidence sha256, with the score marked SEALED, not DECLARED
- `test_training.py` **[rollback]** the promotion recorded what it replaced and the rollback returned to it \x97 a promotion without a way back is a one-way door
...
- `test_live_provider.py` **[wire]** one real HTTP call carried the model, the messages, the configured 4096-token ceiling, exactly the 19 tools this role is allowed, the bearer key and the configured extra header \x97 and with the ceiling left at its default, max_tokens is omitted rather than sent as 0
- `test_live_provider.py` **[cost]** the provider reported 1M+1M tokens and the ledger charged $18.00 at the configured rates \x97 spend is read from the response, never estimated by the client
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.0s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.1s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_live_provider.py` **[malformed]** a non-JSON body and a body with no choices are each retried through the full ladder, then failed over to the configured fallback, and logged against the provider that sent them \x97 they used to raise straight out of the loop, killing the task and never trying the fallback
- `test_live_provider.py` **[timeout]** a provider that hung for 20s was cut off by the 2s ceiling and retried, finishing in 2.0s \x97 the timeout is a real bound, not a suggestion
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname 'e159ecdd6529' which is not this machine's, on a Windows host running python 3.14 \x97 this is not the host backend wearing a different name
- `test_docker_live.py` **[mount]** the expert's root is /work inside the container: a file written there landed on the host, and a file the host wrote was readable inside \x97 in both directions, byte for byte
- `test_docker_live.py` **[containment]** 3 probes for the host filesystem \x97 a drive root, the platform's own source directory, and the fleet home above the mount \x97 all came back empty from inside the container
...
- `test_docker_live.py` **[credentials]** three credential-shaped variables were withheld from the container by name and by value, and of the 11 variables it did receive none came from this host except the image's own \x97 both filters checked, not just the outer one
- `test_docker_live.py` **[timeout]** a 60-second command under a 6-second ceiling was cut off in 7.2s, reported as a failure, and left no container behind
- `test_docker_live.py` **[limits]** every run carries --rm, --memory 1g and --pids-limit 256; asked for 768 processes the container reached 0 and went no further \x97 the ceiling is enforced by the daemon, not merely declared
...
- `test_hosted_sandbox.py` **[no-key]** both hosted backends refuse without a key, name the key as the reason, and \x97 the property that matters \x97 run nothing on this machine instead
...
- `test_hosted_sandbox.py` **[spellings]** a non-zero exit is reported as a failure in both `exitCode` and `exit_code` forms \x97 reading only one would turn every failed remote command into a success
- `test_hosted_sandbox.py` **[failures]** 4 failure shapes \x97 a billing refusal, a server error, a non-JSON body and a host that is not listening \x97 each became a reported non-zero result with a message, and none raised
- `test_hosted_sandbox.py` **[honesty]** with no key each hosted backend reports itself unavailable and names the variable; with a key present it reports itself configured \x97 which is all a key can honestly establish
...
- `test_first_day.py` **[probe-fail]** a rejected key reports FAIL with the HTTP status and exits non-zero; a missing key reports FAIL naming the exact environment variable to set \x97 the two failures a first day actually produces, told apart
...
- `test_first_day.py` **[metered]** all 4 probe call(s) landed in the model-call ledger with their provider and model \x97 `loop.py check` is a real provider call and is now attributed like any other
- `test_first_day.py` **[first-task]** with the probe green, a gated task ran to completion over the same provider \x97 the artefact exists, the gate passed, and the key appears nowhere in 3606 characters of log
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94629 -> 198704 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.14s, 0.16s, 0.15s, 0.16s, 0.16s, 0.16s \x97 median 0.16s, and the last batch is not an outlier: the loop does not get slower as its own history grows
- `test_endurance.py` **[logs]** agent.log is 116 KB and rotates at 5 MB x 5 backups \x97 a hard ceiling of 29 MB per expert, whatever happens
- `test_endurance.py` **[locks]** no lock file survived 120+ tasks and 6 loop restarts \x97 every one was released by its holder or reclaimed as stale
- `test_endurance.py` **[ledgers]** the whole expert directory is 2.0 MB after 120+ tasks (17.1 KB per task): the model gateway 48 KB, routing outcomes 27 KB, compiled context windows 1157 KB
- `test_endurance.py` **[context]** across 42 compiled windows the median size went 1259 -> 1259 tokens: the window is bounded by its budget, not by how much the fleet remembers
- `test_endurance.py` **[soak]** 22s of continuous operation. This is minutes, not weeks: it rules out the growth that is O(total work), and it cannot rule out a leak that needs days to show.
...
- `test_ux.py` **[supervision]** one request answers objective, current action ('read the Acme MSA' -> C1), 2 open criteria, 1 blocker(s), and cost \x97 and the blocker routes to a person rather than to a retry
...
- `test_ux.py` **[training]** ingested / covered / examined / still-open are four separate numbers: 0 source(s), 1/2 requirements evidenced, exam 88% (pass), 1 gap(s) still open \x97 and no percentage is computed anywhere without its denominator
...
- `test_universal.py` **[sources]** the same claim was accepted at tier 1 from an RFC and refused at tier 3 from a content farm \x97 what is believed depends on where it came from, decided by rule and never by a model
...
- `test_universal.py` **[authority-corpus]** 28 phrasings that must reach the owner all do \x97 including 'log into', 'sign in to', 'authenticate' and 'credentials', which the hand-written whole-word table missed \x97 and 10 ordinary goals are still not escalated
...
- `test_universal.py` **[route]** 15 goal shapes each landed on the declared system, every verdict carrying a why, a CLI path and a panel path \x97 and the note still says it is a mechanical floor, not understanding
...
- `test_universal.py` **[unified]** POST /api/achieve is reachable, permissioned 'run' by declaration, validates its inputs, and REFUSED to start a goal needing the owner \x97 naming 2 blocker(s) instead of beginning. The layer that orchestrates everything is no longer reachable only from a terminal.
...
- `test_grants.py` **[lifetime]** a grant stops working the day it expires and the moment it is revoked \x97 a permission that outlives its reason is how a temporary exception becomes standing access nobody approved
...
- `test_grants.py` **[ledger]** the cap is consumed by real use, a second spend that would breach it is refused, and every use is written to a ledger \x97 a standing grant without a usage log is a blank cheque
- `test_grants.py` **[wired]** universal.assess honours a live grant \x97 the credential gap clears for the granted vendor, the MONEY gap still blocks, and another vendor is still blocked; nothing self-grants, and the moment the grant expires or is revoked this reverts with no code change
- `test_grants.py` **[recorded]** acting under 2 standing grant(s) wrote 2 usage row(s) naming the expert and the work, while a read-only assessment wrote none \x97 the difference between a permission and a blank cheque is the log of what was done with it
- `test_experience.py` **[gotchas]** a rookie doing the same work inherits a sibling's environment failure \x97 attributed, and marked as a WARNING rather than the binding rule it is for the expert that paid for it
- `test_experience.py` **[shared]** a rookie with no history of its own inherited a sibling's FIXED case for the same work \x97 attributed, dated, and marked as verified in that expert's environment rather than this one's; an unrelated expert's unrelated case was not injected; and no expert harvests itself
