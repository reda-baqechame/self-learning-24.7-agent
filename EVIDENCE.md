...
Generated 2026-09-09T18:41:20 from an actual suite run: **157/160 tests passed**, **922 observations** recorded.
...
| 5. Improvement & governance | **proven except skipped** | 25/26 | 156 |
...
- `test_resume.py` **[phase 1]** killed mid-task after step 0, status=running
- `test_resume.py` **[phase 2]** resumed and completed: 5 steps, status=done
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13410 token window in 1.5s, cut marked
...
- `test_retention.py` **[flat]** the hot state is capped at 50 finished task(s) after 400 more were run (limit 65), so persist is bounded work forever — measured 11 ms -> 13 ms here, but the COUNT is the guarantee and the clock is only a smoke check
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 22709m > 1m
...
- `test_runbook.py` **[goal g-20260909-182858]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260909-182858]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260909-183207]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260909-183207]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
**Verdict: proven except skipped** — 25 of 26 declared tests ran and passed, producing 156 observations.
...
- `test_package.py` **[runnable]** the archive carries 128 non-test Python files (including nested support scripts), 160 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 160 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 80,071 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 160 tests and 216 mutations
...
- `test_invariants.py` **[execution]** 124 modules scanned; 0 raw subprocess sites outside the authority (19 declared platform-internal, each with a stated reason)
...
- `test_invariants.py` **[metering]** 124 modules scanned by AST; every function that reaches a model provider meters it (2 declared free, each with a stated reason and each still present in the source)
...
- `test_invariants.py` **[sandboxes]** 195 sandbox names across 160 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 124 modules, 160 acceptance tests
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 142 ms per command — 27 s before the caches — and a change to a cached path is still caught
...
- `test_live_provider.py` **[wire]** one real HTTP call carried the model, the messages, the configured 4096-token ceiling, exactly the 19 tools this role is allowed, the bearer key and the configured extra header — and with the ceiling left at its default, max_tokens is omitted rather than sent as 0
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.5s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.1s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '127e68766cbf' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_docker_live.py` **[timeout]** a 60-second command under a 6-second ceiling was cut off in 7.6s, reported as a failure, and left no container behind
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94634 -> 198701 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.15s, 0.17s, 0.18s, 0.18s, 0.18s, 0.18s — median 0.18s, and the last batch is not an outlier: the loop does not get slower as its own history grows
