...
Generated 2026-09-08T10:34:30 from an actual suite run: **156/159 tests passed**, **910 observations** recorded.
...
| 6. Control plane & interop | **proven** | 22/22 | 148 |
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13410 token window in 1.7s, cut marked
...
- `test_retention.py` **[flat]** the hot state is capped at 50 finished task(s) after 400 more were run (limit 65), so persist is bounded work forever — measured 11 ms -> 18 ms here, but the COUNT is the guarantee and the clock is only a smoke check
...
- `test_team.py` **[team t-test]** done -> C:\tmp\t6full2\team\teamwork\t-test\result.md
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 20783m > 1m
...
- `test_contract.py` **[goal g-accept]** turn this success into deterministic capability: python runbook.py draft C:\tmp\t6full2\contract\experts\honest-builder g-accept (then fill the TODO steps and let three verified runs promote it)
...
- `test_runbook.py` **[goal g-20260908-102329]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260908-102329]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260908-102419]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260908-102419]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
**Verdict: proven** — 22 of 22 declared tests ran and passed, producing 148 observations.

<details><summary>What the tests observed (148)</summary>
...
- `test_package.py` **[runnable]** the archive carries 126 non-test Python files (including nested support scripts), 159 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 79,987 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 159 tests and 127 mutations
...
- `test_invariants.py` **[execution]** 122 modules scanned; 0 raw subprocess sites outside the authority (19 declared platform-internal, each with a stated reason)
...
- `test_invariants.py` **[metering]** 122 modules scanned by AST; every function that reaches a model provider meters it (2 declared free, each with a stated reason and each still present in the source)
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 122 modules, 159 acceptance tests
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 187 ms per command — 27 s before the caches — and a change to a cached path is still caught
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.4s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.2s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '1131a580c5e9' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_first_day.py` **[bootstrap]** an empty directory became a fleet with an expert ('first-day') in one command; the key reached agent.env and appears nowhere in 1663 characters of output
...
- `test_first_day.py` **[unreachable]** a base_url nothing is listening on reported FAIL in 2.8s instead of hanging on a 20-second timeout per role
...
- `test_first_day.py` **[first-task]** with the probe green, a gated task ran to completion over the same provider — the artefact exists, the gate passed, and the key appears nowhere in 3575 characters of log
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94629 -> 198661 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.19s, 0.18s, 0.21s, 0.19s, 0.20s, 0.19s — median 0.19s, and the last batch is not an outlier: the loop does not get slower as its own history grows
...
- `test_endurance.py` **[soak]** 27s of continuous operation. This is minutes, not weeks: it rules out the growth that is O(total work), and it cannot rule out a leak that needs days to show.
