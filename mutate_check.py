...
MUTATIONS = [
    ("computer postcondition: duplicate action identity accepted", "computerverify.py",
     ...
     "action provenance must be unique across the complete lineage census"),
    ("computerbench: refused audit click counted as required", "computerbench.py",
     ...
     "a refused predispatch audit attempt cannot invalidate verified frozen actions"),
    ("computerbench: current links follow page order", "computerbench.py",
     "    return ordered\n\n\ndef _environment(session, seen):",
     "    return list(links)\n\n\ndef _environment(session, seen):",
     "test_computerbench.py",
     "current links must be resolved in caller-frozen intent order"),
    ("computerbench: pre-record restart refusal called unresolved", "computerbench.py",
     "                   for action in actions)):\n        return \"unresolved\"\n    clicks = [action for action in actions if action.get(\"operation\") == \"click\"]",
     "                   for action in actions)\n            or (attempted and not any(action.get(\"operation\") == \"click\"\n                                      for action in actions))):\n        return \"unresolved\"\n    clicks = [action for action in actions if action.get(\"operation\") == \"click\"]",
     "test_computerbench.py",
     "a controller refusal before durable preparation has no click record"),
    ("computer postcondition: refused attempt counted as required action", "computerverify.py",
     "            actions=[by_id[action_id] for action_id in action_ids]",
     "            actions=[a for a in ledger['actions'] if a.get('workflow_id')==workflow_id]",
     "test_computer_postconditions.py",
     "completion must use the committed ordered action ids, not refused audit attempts"),
    ("computer postcondition: predispatch refusal consumes intent", "computersession.py",
     "                          intent_index=position,required_for_task=True)\n        data['actions'].append(action)",
     "                          intent_index=position,required_for_task=True)\n            workflow.setdefault('action_ids',[]).append(action['id'])\n        data['actions'].append(action)",
     "test_computer_postconditions.py",
     "a known predispatch refusal must leave the frozen intent retryable"),
    ("computerbench: verified-empty safe refusal called unresolved", "computerbench.py",
     "    if outcome == \"refused\":\n        return \"safe_refusal\"\n    if record[\"artifact_outcome\"] == \"verified\":",
     "    if False:\n        return \"safe_refusal\"\n    if record[\"artifact_outcome\"] == \"verified\":",
     "test_computerbench.py",
     "a controller refusal with no unresolved effect remains a safe refusal"),
    ("computer postcondition: prepared output claim never committed", "computersession.py",
     "            computerverify.commit_output_claim(self.root,claim)",
     "            if False: computerverify.commit_output_claim(self.root,claim)",
     "test_computer_postconditions.py",
     "verification requires a durably COMMITTED output claim"),
    ("computer postcondition: root-wide output claim not persisted", "computersession.py",
     "            computerverify.write_output_claim(self.root,claim)",
     "            if False: computerverify.write_output_claim(self.root,claim)",
     "test_computer_postconditions.py",
     "different ledgers must observe one durable physical output owner"),
    ("computer postcondition: recovery accepts tampered workflow contract", "computerverify.py",
     "        if (canonical_digest(contract)!=claim['contract_sha256']",
     "        if (False",
     "test_computer_postconditions.py",
     "prepared-claim recovery must rehash the validated workflow contract"),
    ("computer postcondition: same-session output claim reused", "computersession.py",
     "        if owner is not None and owner!=claimant:\n            raise C.Refused('invoice output authority belongs to another workflow')",
     "        if False:\n            raise C.Refused('invoice output authority belongs to another workflow')",
     "test_computer_postconditions.py",
     "one physical output authority can belong to only one active workflow"),
    ("computer postcondition: malformed baseline dereferenced", "computerverify.py",
     "    if not isinstance(baseline,dict) \\",
     "    if False \\", "test_computer_postconditions.py",
     "malformed durable contracts must return fail-closed rather than escape completion"),
    ("computer postcondition: output lease uses path spelling", "computersession.py",
     "        key=computerverify.output_lease_rel(binding)",
     "        key=computerverify.output_lease_rel({'device':0,'inode':int(computerverify.canonical_digest(output_dir)[:12],16)})",
     "test_computer_postconditions.py",
     "Windows aliases for one physical output directory must contend on one lease",
     False, None, ("nt", "Windows filesystem alias identity")),
    ("computer postcondition: durable cleanup equality ignored", "computerverify.py",
     "    if (not isinstance(cleanup,dict) or cleanup!=action.get('cleanup_receipt')",
     "    if (not isinstance(cleanup,dict) or False",
     "test_computer_postconditions.py",
     "the receipt cleanup must equal the action's durable cleanup history"),
    ("computer postcondition: cleanup environment digest ignored", "computerverify.py",
     "            or canonical_digest(environment)!=cleanup['metadata_sha256']:",
     "            or False:", "test_computer_postconditions.py",
     "cleanup evidence must bind the durable closed environment record"),
    ("computer postcondition: action contract binding ignored", "computerverify.py",
     "            and action.get('contract_sha256')==workflow.get('contract_sha256')",
     "            and True", "test_computer_postconditions.py",
     "the action's durable contract digest must match its receipt and workflow"),
    ("computer postcondition: protected output zone bypassed", "computerverify.py",
     "    if fileauth.zone_of(relative) not in (fileauth.ZONE_CONTROL,\n                                          fileauth.ZONE_RUNTIME):",
     "    if False:", "test_computer_postconditions.py",
     "invoice evidence must be written outside ordinary agent file authority"),
    ("computer postcondition: baseline captured before output lease", "computersession.py",
     "            held_binding,held_key=self._hold_output_path(output_dir)",
     "            held_binding,held_key=output_binding,output_key",
     "test_computer_postconditions.py",
     "the output-authority lease must be held before baseline capture"),
    ("computer postcondition: unresolved UNKNOWN ignored at completion", "computerverify.py",
     "                if action.get('state') in ('PREPARED','DISPATCHED','UNKNOWN')",
     "                if action.get('state') in ('PREPARED','DISPATCHED')",
     "test_computer_postconditions.py",
     "every unresolved critical UNKNOWN action must block finish_task"),
    ("computer postcondition: ambiguous refusal becomes conclusive failure", "computersession.py",
     "                if isinstance(error,computerverify.PredicateMismatch)",
     "                if isinstance(error,C.Refused)",
     "test_computer_postconditions.py",
     "deadline and verifier ambiguity must never become FAILED_POSTCONDITION"),
    ("computer postcondition: receipt intent binding ignored", "computerverify.py",
     "            and receipt.get('intent_key')==action.get('intent_key')",
     "            and True", "test_computer_postconditions.py",
     "a recomputed receipt for another intent cannot authorize another effect"),
